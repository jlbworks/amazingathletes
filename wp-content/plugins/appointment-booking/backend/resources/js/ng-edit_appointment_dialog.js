        ...
        $scope.processForm = function() {
            ...
            $scope.form.customers.forEach(function (item, i, arr) {
                ...
                if ($scope.form.service) {
                    jQuery('#ab_custom_fields_dialog label.service_' + $scope.form.service.id + ' input:checkbox').each(function () {
                        service_extras.push(jQuery(this).val());
                    });
                }
...
var showAppointmentDialog = function (appointment_id, staff_id, start_date, end_date, callback) {
    ...
    $scope.$apply(function ($scope) {
        ...
        if (appointment_id) {
            ...
            $modal_title.text(BooklyL10n.editAppointment);
        } else {
            $scope.configureNewForm(staff_id, start_date, callback);
            $modal_title.text(BooklyL10n.newAppointment);
        }
    });
