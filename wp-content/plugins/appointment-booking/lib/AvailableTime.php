            ...
            if ( isset ( $staff['working_hours'][ $day_of_week ] ) && $this->isWorkingDay( $date, $staff_id ) ) {
                if ( $is_all_day_service ) {
                    ...
                    if ( $date == $current_date ) {
                        ...
                        $new_start = $working_start + ceil ( ( $current_timestamp % 86400 - $working_start ) / $time_slot_length ) * $time_slot_length;
                        ...
                        if ( array_key_exists( 'bookings', $this->staffData[ $cart_item['staff_id'] ] ) ) {
                            ...
                            foreach ( $this->staffData[ $cart_item['staff_id'] ]['bookings'] as &$booking ) {
                                ...
                                if ( $booking['service_id'] == $cart_item['service_id']
                                    ...
                                    // Add persons in staff bookings appointment from cart.
                            ...
                            if ( ! $exist_booking ) {
                                ...
                                $this->staffData[ $cart_item['staff_id'] ]['bookings'][] = array(
                                    'staff_id'           => $cart_item['staff_id'],
                                    'service_id'         => $cart_item['service_id'],
                                    'start_date'         => $s_date,
                                    'extras'             => json_encode( $cart_item['extras'] ),
