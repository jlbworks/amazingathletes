...
function submit_data() {
    ...
    if ($_POST['form_handler'] == 'edit_coach_invoice') {
        ...
        $meta_data = array();
        $meta_fields = array(
            'total', 'subtotal','grand_total', 'other','notes', 'travel_surcharge', 'liability_insurance_rebate', 'equipment_rental_rebate', 'settled_outstanding_student_compensations','status'
        );
        foreach ( $meta_fields as $field ) {
            $meta_data[$field] = str_replace('$','',sanitize_text_field($_POST[$field]));
        }
